...
void COScopeCtrl::InvalidateCtrl(bool deleteGraph)
{
   ...
   yUnitFont.CreateFont(12, 0, 900, 900, FW_NORMAL, FALSE, FALSE, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, _T("MS Shell Dlg"));
   ...
   m_dcGrid.TextOut((m_rectClient.left + m_rectPlot.left - 8) / 2 - rText.Height() / 2, (m_rectPlot.bottom + m_rectPlot.top) / 2 - rText.Height() / 2, m_str.YUnits);
   m_dcGrid.SelectObject(oldFont);
...
void COScopeCtrl::AppendEmptyPoints(double dNewPoint[], bool bInvalidate, bool bAdd2List, bool bUseTrendRatio)
{
   ...
   if (m_dcPlot.GetSafeHdc() != NULL)
   {
      ...
      for (iTrend = 0; iTrend < m_NTrends; iTrend ++)
      {
         currY                                           = m_rectPlot.bottom - (long)((m_PlotData[iTrend].dCurrentPosition - m_PlotData[iTrend].dLowerLimit) * m_PlotData[iTrend].dVerticalFactor);
...
void COScopeCtrl::DrawPoint()
{
   ...
   if (m_dcPlot.GetSafeHdc() != NULL)
   {
      ...
      for (iTrend = 0; iTrend < m_NTrends; iTrend ++)
      {
         ...
         {
            prevY                                        = m_rectPlot.bottom - (long)((m_PlotData[iTrend].dPreviousPosition - m_PlotData[iTrend].dLowerLimit) * m_PlotData[iTrend].dVerticalFactor);
         }
